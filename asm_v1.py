...
class AbsRegisterManager(ABC):
    ...
    def set_defaults(self, kwargs):
        ...
        if self.defaults:
            ...
            raise RuntimeError(
                "%s already has a set of default parameters" % (self.ch_name)
            )
    ...
    def set_registers(self, kwargs):
        ...
        if not self.defaults.keys().isdisjoint(kwargs):
            raise RuntimeError(
                "these params were set for {0} both in default_pulse_registers and set_pulse_registers: {1}".format(
                    self.ch_name, self.defaults.keys() & kwargs.keys()
                )
            )
...
class ReadoutManager(AbsRegisterManager):
    ...
    def write_regs(self, params, defaults):
        ...
        if not defaults:
            ...
            mc = self.get_mode_code(
                phrst=phrst, mode=mode, outsel=outsel, length=params["length"]
            )
            self.set_reg(
                "mode",
                mc,
    ...
    def get_mode_code(self, length, outsel=None, mode=None, phrst=None):
        ...
        if length >= 2**16 or length < 3:
            raise RuntimeError(
                "Pulse length of %d is out of range (exceeds 16 bits, or less than 3) - use multiple pulses, or zero-pad the waveform"
                % (length)
            )
        if outsel is None:
            outsel = "product"
    ...
    def write_regs(self, params, defaults):
        ...
        * flat_top: A flattop pulse with arbitrary ramps.
        ...
        if not defaults:
            style = params["style"]
            # these mode bits could be defined, or left as None
            phrst, stdysel, mode, outsel = [
                params.get(x) for x in ["phrst", "stdysel", "mode", "outsel"]
            ]
            if phrst is not None and self.gencfg["type"] != "axis_signal_gen_v6":
                raise RuntimeError(
                    "phrst not supported for %s, only for axis_signal_gen_v6"
                    % (self.gencfg["type"])
                )
            ...
            if style == "const":
                ...
                self.next_pulse["regs"].append(
                    [
                        self.regmap[(self.ch, x)][1]
                        for x in ["freq", "phase", "0", "gain", "mode"]
                    ]
                )
                ...
                self.next_pulse["regs"].append(
                    [
                        self.regmap[(self.ch, x)][1]
                        for x in ["freq", "phase", "addr", "gain", "mode"]
                    ]
                )
                ...
                self.next_pulse["regs"].append(
                    [
                        self.regmap[(self.ch, x)][1]
                        for x in ["freq", "phase", "addr", "gain", "mode2"]
                    ]
                )
                self.next_pulse["regs"].append(
                    [
                        self.regmap[(self.ch, x)][1]
                        for x in ["freq", "phase", "0", "gain2", "mode"]
                    ]
                )
                self.next_pulse["regs"].append(
                    [
                        self.regmap[(self.ch, x)][1]
                        for x in ["freq", "phase", "addr2", "gain", "mode3"]
                    ]
                )
        ...
        * flat_top: A flattop pulse with arbitrary ramps.
        ...
        if "phase" in params and "freq" in params:
            ...
            self.set_reg(
                "freq",
                (phase << 16) | freq,
                f"phase = {phase} | freq = {freq}",
                defaults=defaults,
            )
        if "gain" in params and (
            "waveform" in params or params.get("style") == "const"
        ):
            gain = params["gain"]
            self.set_reg(
                "addr",
                (gain << 16) | addr,
                f"gain = {gain} | addr = {addr}",
                defaults=defaults,
            )
        if not defaults:
            style = params["style"]
            ...
            if style == "const":
                ...
                self.next_pulse["regs"].append(
                    [
                        self.regmap[(self.ch, x)][1]
                        for x in ["freq", "addr", "mode", "0", tmux_reg]
                    ]
                )
                self.next_pulse["length"] = params["length"]
            elif style == "arb":
                ...
                self.set_reg(
                    "addr", (gain << 16) | addr, f"gain = {gain} | addr = {addr}"
                )
                # gain+addr for flat
                self.set_reg(
                    "gain",
                    (int(gain * maxv_scale / 2) << 16),
                    f"gain = {gain} | addr = {addr}",
                )
                # gain+addr for ramp-down
                self.set_reg(
                    "addr2",
                    (gain << 16) | addr + (wfm_length + 1) // 2,
                    f"gain = {gain} | addr = {addr}",
                )
                ...
                self.next_pulse["regs"].append(
                    [
                        self.regmap[(self.ch, x)][1]
                        for x in ["freq", "addr", "mode2", "0", tmux_reg]
                    ]
                )
                self.next_pulse["regs"].append(
                    [
                        self.regmap[(self.ch, x)][1]
                        for x in ["freq", "gain", "mode", "0", tmux_reg]
                    ]
                )
                self.next_pulse["regs"].append(
                    [
                        self.regmap[(self.ch, x)][1]
                        for x in ["freq", "addr2", "mode2", "0", tmux_reg]
                    ]
                )
    ...
    def write_regs(self, params, defaults):
        ...
        if "length" in params:
            if params["length"] >= 2**32 or params["length"] < 3:
                raise RuntimeError(
                    "Pulse length of %d is out of range (exceeds 32 bits, or less than 3) - use multiple pulses"
                    % (params["length"])
                )
        ...
        if "mask" in params:
            ...
            for maskch in mask:
                if maskch not in range(self.gencfg["n_tones"]):
                    raise RuntimeError(
                        "invalid mask specification: tone %d was requested, but is out of range for this mux generator"
                    )
                val_mask |= 1 << maskch
        ...
        if not defaults:
            ...
            self.next_pulse["regs"] = []
            self.next_pulse["regs"].append(
                [self.regmap[(self.ch, x)][1] for x in ["freq", "phase", "0", "0", "0"]]
            )
            self.next_pulse["length"] = params["length"]
...
class QickProgram(AbsQickProgram):
    ...
    instructions = {
        ...
        "mathi": {
            "type": "I",
            "bin": 0b00010010,
            "fmt": ((0, 53), (1, 41), (2, 36), (3, 46), (4, 0)),
            "repr": "{0}, ${1}, ${2} {3} {4}",
        },
        "seti": {
            "type": "I",
            "bin": 0b00010011,
            "fmt": ((1, 53), (0, 50), (2, 36), (3, 0)),
            "repr": "{0}, {1}, ${2}, {3}",
        },
        ...
        "condj": {
            "type": "J2",
            "bin": 0b00110001,
            "fmt": ((0, 53), (2, 46), (1, 36), (3, 31), (4, 0)),
            "repr": "{0}, ${1}, {2}, ${3}, @{4}",
        },
        "math": {
            "type": "R",
            "bin": 0b01010000,
            "fmt": ((0, 53), (3, 46), (1, 41), (2, 36), (4, 31)),
            "repr": "{0}, ${1}, ${2} {3} ${4}",
        },
        "set": {
            "type": "R",
    ...
    gentypes = {
        "axis_signal_gen_v4": FullSpeedGenManager,
    ...
    def __init__(self, soccfg):
        ...
        if self.tproccfg["type"] != "axis_tproc64x32_x8":
            raise RuntimeError(
                "tProc v1 programs can only be run on a tProc v1 firmware"
            )
        ...
        self._gen_mgrs = [
            ...
            for iCh, ch in enumerate(soccfg["gens"])
        ]
        self._ro_mgrs = [
            ReadoutManager(self, iCh) if "tproc_ctrl" in ch else None
            for iCh, ch in enumerate(soccfg["readouts"])
        ]
        ...
        for ch in ch_list:
            ...
            if t is not None:
                ...
                if t == "auto":
                    ...
                    logger.warning(
                        "pulse time %d appears to conflict with previous pulse ending at %f?"
                        % (t, ts)
                    )
                else:
                    t_ch = int(t)
                ...
                pulse_length *= (
                    self.tproccfg["f_time"] / self.soccfg["gens"][ch]["f_fabric"]
                )
    ...
    def trigger(
        ...
        if any([adcs, ddr4, mr]):
            ...
            for ro in adcs:
                ...
                ro_length *= (
                    self.tproccfg["f_time"] / self.soccfg["readouts"][ro]["f_output"]
                )
    ...
    def reset_phase(
        self,
        gen_ch: Union[int, List[int]] = None,
        ro_ch: Union[int, List[int]] = None,
        t: int = 0,
    ):
        """
        ...
        for ch_type, ch_list in channels.items():
            for ch in ch_list:
                ...
                if ch_type == "generator":
                    ...
                    if t < ts:
                        logger.warning(
                            f"generator {ch} phase reset at t={t} appears to conflict "
                            f"with previous pulse ending at {ts}"
                        )
                    ch_mgr = self._gen_mgrs[ch]
                    phrst_params = dict(
                        style="const", phase=0, freq=0, gain=0, length=3, phrst=1
                    )
                    tproc_ch = self.soccfg["gens"][ch]["tproc_ch"]
                    ...
                    if t < ts:
                        logger.warning(
                            f"readout {ch} phase reset at t={t} appears to conflict "
                ...
                for regs in ch_mgr.next_pulse["regs"]:
                    self.set(
                        ...
                        *regs,
                        r_t,
                        f" {ch_type} ch{ch} phase reset @t = ${r_t}",
                    )
    ...
    def compile(self, debug=False):
        ...
        for inst in self.prog_list:
            ...
            prog_counter += 1
        self.binprog = [
            self.compile_instruction(inst, labels, debug=debug)
            for inst in self.prog_list
            if inst["name"] != "comment"
        ]
    ...
    def _inst2asm(self, inst, max_label_len):
        if inst["name"] == "comment":
            return "// " + inst["comment"]
        template = (
            inst["name"] + " " + self.__class__.instructions[inst["name"]]["repr"] + ";"
        )
        ...
        if "comment" in inst:
            line += (
                " " * (48 - len(line))
                + "//"
                + (inst["comment"] if inst["comment"] is not None else "")
            )
        if "label" in inst:
            label = inst["label"]
...
class QickRegister:
    ...
    def set_to(
        self,
        a: Union["QickRegister", float, int],
        operator: str = "+",
        b: Union["QickRegister", float, int] = 0,
        physical_unit=True,
    ):
        """
        ...
        if (
            type(a) != QickRegister
        ):  # assign value "a" to register, do unit conversion if physical_unit==True
            reg = self.val2reg(a) if physical_unit else a
            comment = f"'{self.name}' <= {reg} " + (
                f"({a} {self.unit})"
                if physical_unit and (self.unit is not None)
                else ""
            )
            ...
            if type(b) == QickRegister:
                ...
                if not (self.page == a.page == b.page):
                    raise RuntimeError(
                        f"the qick registers for mathematical operation must be on the same page. "
                ...
                reg = (
                    self.val2reg(b) if physical_unit else b
                )  # do unit conversion on "b" if physical_unit==True
...
class QickRegisterManagerMixin:
    ...
    def new_reg(
        ...
        if addr is None:
            ...
            if addr < 1 or addr > 12:
                raise ValueError(
                    f"register address must be greater than 0 and smaller than 13"
                )
            if (page, addr) in self._user_regs:
                raise ValueError(
                    f"register at address {addr} in page {page} is already occupied."
                )
        ...
        reg = QickRegister(
            self, page, addr, reg_type, gen_ch, ro_ch, init_val, name=name
        )
    ...
    def new_gen_reg(
        ...
        if tproc_reg:
            ...
            return self.new_reg(
                page, addr, name, init_val, reg_type, gen_ch, gen_cgf.get("ro_ch")
            )
