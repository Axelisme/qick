...
class QickParam:
    ...
    def __float__(self):
        if self.is_sweep():
            raise RuntimeError(
                "tried to cast a swept QickParam to float, which is not safe"
            )
        return self.start
    ...
    def to_int(self, scale, quantize, parname, trunc=False):
        ...
        start = to_int(
            self.start, scale, quantize=quantize, parname=parname, trunc=trunc
        )
        spans = {
            k: to_int(v, scale, quantize=quantize, parname=parname, trunc=trunc)
            for k, v in self.spans.items()
        }
        self.raw_param = QickRawParam(
            par=parname, start=start, spans=spans, quantize=quantize
        )
    ...
    def get_rounded(self, loop_counts: dict[str, int] = None) -> QickParam:
        ...
        raise RuntimeError(
            "to_int has not been called on this QickParam or its descendants"
        )
...
class QickRawParam(SimpleClass):
    ...
    def __int__(self):
        if self.is_sweep():
            raise RuntimeError(
                "tried to cast a swept QickRawParam to int, which is not safe"
            )
        return self.start
    ...
    def to_steps(self, loops):
        ...
        for loop, r in self.spans.items():
            ...
            if nSteps == 1 or r == 0:
                ...
                stepsize = int(
                    self.quantize * np.trunc(r / (nSteps - 1) / self.quantize)
                )
                if stepsize == 0:
                    raise RuntimeError(
                        "requested sweep step is smaller than the available resolution: span=%d, steps=%d"
                        % (r, nSteps - 1)
                    )
    ...
    def __copy__(self):
        newparam = QickRawParam(
            self.par, self.start, copy.copy(self.spans), self.quantize
        )
    ...
    def __imul__(self, a):
        ...
        if self.steps is not None:
            raise RuntimeError(
                "QickRawParam can only be multiplied before steps have been defined"
            )
        if isinstance(a, Fraction):
            if not all(
                [
                    x % a.denominator == 0
                    for x in [self.start, self.quantize] + list(self.spans.values())
                ]
            ):
    ...
    def __truediv__(self, a):
        ...
        if self.steps is None:
            raise RuntimeError(
                "QickRawParam can only be divided after steps have been defined"
            )
...
class Waveform(Mapping, SimpleClass):
    ...
    def __init__(
        self,
        freq: Union[int, QickRawParam],
        phase: Union[int, QickRawParam],
        env: int,
        gain: Union[int, QickRawParam],
        length: Union[int, QickRawParam],
        conf: int,
        name: str = None,
    ):
        self.freq = freq
    ...
    def compile(self):
        ...
        rawbytes = b"".join(
            [
                int(i % 2 ** (8 * w)).to_bytes(
                    length=w, byteorder="little", signed=False
                )
                for i, w in zip(startvals, self.widths)
            ]
        )
    ...
    def sweeps(self):
        return [
            r
            for r in [self.freq, self.phase, self.gain, self.length]
            if isinstance(r, QickRawParam)
        ]
...
class QickPulse(SimpleClass):
    ...
    SPECIAL_WAVEFORMS = {
        "dummy": Waveform(
            freq=0, phase=0, env=0, gain=0, length=3, conf=0, name="dummy"
        ),
        "phrst": Waveform(
            freq=0, phase=0, env=0, gain=0, length=3, conf=0b010000, name="phrst"
        ),
    }

    def __init__(
        self, prog: "QickProgramV2", ch_mgr: "AbsRegisterManager", params: dict = {}
    ):
        self.prog = prog
        self.ch_mgr = ch_mgr
        if ch_mgr is None:
            ...
            self.numeric_params = list(params.keys() & ch_mgr.PARAMS_NUMERIC) + [
                "total_length"
            ]
        self.params = params
        self.waveforms = []
    ...
    def add_wave(self, waveform):
        ...
        if not isinstance(waveform, Waveform):
            ...
            if waveform in self.prog.wave2idx:
                ...
                raise RuntimeError(
                    "add_wave argument {waveform} is neither a Waveform nor a waveform name"
                )
    ...
    def get_length(self):
        ...
        if self.ch_mgr is None:
            logger.warning(
                "no channel manager defined for this pulse, get_length() will return 0"
            )
        else:
...
class WriteReg(Macro):
    ...
    def expand(self, prog):
        ...
        if isinstance(self.src, str):
            ...
            return [
                AsmInst(
                    inst={"CMD": "REG_WR", "DST": dst, "SRC": "op", "OP": src},
                    addr_inc=1,
                )
            ]
...
class ReadDmem(Macro):
    ...
    def expand(self, prog):
        ...
        return [
            AsmInst(
                inst={"CMD": "REG_WR", "DST": dst, "SRC": "dmem", "ADDR": addr},
                addr_inc=1,
            )
        ]
...
class WriteDmem(Macro):
    ...
    def expand(self, prog):
        ...
        if isinstance(self.src, str):
            ...
            return [
                AsmInst(
                    inst={"CMD": "DMEM_WR", "DST": dst, "SRC": "op", "OP": src},
                    addr_inc=1,
                )
            ]
        raise RuntimeError(f"invalid src: {self.src}")
...
class CondJump(Macro):
    ...
    def expand(self, prog):
        ...
        if self.arg2 is not None:
            ...
            insts.append(
                AsmInst(
                    inst={"CMD": "TEST", "OP": " ".join([arg1, op, arg2]), "UF": "1"},
                    addr_inc=1,
                )
            )
        else:
            ...
            insts.append(
                AsmInst(inst={"CMD": "TEST", "OP": arg1, "UF": "1"}, addr_inc=1)
            )
        insts.append(
            AsmInst(
                inst={"CMD": "JUMP", "IF": self.test, "LABEL": self.label}, addr_inc=1
            )
        )
        return insts
...
class CloseLoop(Macro):
    def expand(self, prog):
        ...
        for reg in prog.reg_dict.values():
            # skip zero sweeps
            if (
                isinstance(reg.init, QickRawParam)
                and lname in reg.init.spans
                and reg.init.steps[lname]["step"] != 0
            ):
        ...
        insts.append(
            AsmInst(inst={"CMD": "JUMP", "LABEL": label, "IF": "NZ"}, addr_inc=1)
        )
        ...
        for reg, steps in reg_sweeps:
            insts.append(
                IncReg(dst=reg.full_addr(), src=-steps["step"] - steps["span"])
            )
...
class TimedMacro(Macro):
    ...
    def get_time_param(self, name):
        if name not in self.t_params:
            raise RuntimeError(
                "invalid parameter name; use list_time_params() to get the list of valid names for this instruction"
            )
...
class Delay(TimedMacro):
    ...
    def expand(self, prog):
        ...
        if t_reg is None:
            ...
            return [
                AsmInst(
                    inst={"CMD": "TIME", "C_OP": "inc_ref", "LIT": f"#{t_reg}"},
                    addr_inc=1,
                )
            ]
        else:
            return [
                AsmInst(
                    inst={"CMD": "TIME", "C_OP": "inc_ref", "R1": prog._get_reg(t_reg)},
                    addr_inc=1,
                )
            ]
...
class Wait(TimedMacro):
    ...
    def preprocess(self, prog):
        ...
        if wait.is_sweep():
            ...
            if not self.no_warn:
                logger.warning(
                    "WAIT can only take a scalar argument, but in this case it would be %s, so rounding up to the max val of %f."
                    % (wait, waitmax)
                )
            wait = waitmax
...
class Pulse(TimedMacro):
    ...
    def preprocess(self, prog):
        ...
        if t == "auto":
            ...
            if t < ts:
                logger.warn(
                    "warning: pulse time %s appears to conflict with previous pulse ending at %s?"
                    % (t, ts)
                )
    ...
    def preprocess(self, prog):
        ...
        for ro in self.ros:
            ...
            if ro not in prog.ro_chs:
                raise RuntimeError(
                    "RO channel %d is triggered but never declared" % (ro)
                )
            if rocfg["trigger_type"] == "dport":
                self.outdict[rocfg["trigger_port"]] |= 1 << rocfg["trigger_bit"]
            else:
    ...
    def expand(self, prog):
        ...
        if self.trigset:
            for outport in self.trigset:
                insts.append(
                    AsmInst(
                        inst={"CMD": "TRIG", "SRC": "set", "DST": str(outport)},
                        addr_inc=1,
                    )
                )
        ...
        if self.trigset:
            for outport in self.trigset:
                insts.append(
                    AsmInst(
                        inst={"CMD": "TRIG", "SRC": "clr", "DST": str(outport)},
                        addr_inc=1,
                    )
                )
        return insts
...
class AsmV2:
    ...
    def wait_auto(self, t=0, gens=False, ros=True, tag=None, no_warn=False):
        ...
        """
        self.append_macro(
            Wait(t=t, auto=True, gens=gens, ros=ros, tag=tag, no_warn=no_warn)
        )
    ...
    def trigger(
        ...
        """
        self.append_macro(
            Trigger(ros=ros, pins=pins, t=t, width=width, ddr4=ddr4, mr=mr, tag=tag)
        )
...
class AbsRegisterManager(ABC):
    ...
    def cfg2reg(self, outsel, mode, stdysel, phrst):
        ...
        cfgreg = (
            phrst * 0b010000 + stdysel_reg * 0b01000 + mode_reg * 0b00100 + outsel_reg
        )
...
class AbsGenManager(AbsRegisterManager):
    ...
    def check_params(self, params):
        ...
        check_keys(
            params.keys(), self.PARAMS_REQUIRED[style], self.PARAMS_OPTIONAL[style]
        )
    ...
    def params2wave(
        ...
        if isinstance(lenreg, QickRawParam):
            if lenreg.maxval() >= 2**16 or lenreg.minval() < 3:
                raise RuntimeError(
                    "Pulse length of %d cycles is out of range (exceeds 16 bits, or less than 3) - use multiple pulses, or zero-pad the envelope"
                    % (lenreg)
                )
        else:
    ...
    def params2pulse(self, par):
        ...
        * flat_top: A flattop pulse with arbitrary ramps.
        ...
        if par.get("phrst") is not None and self.chcfg["type"] not in phrst_gens:
            raise RuntimeError(
                "phrst not supported for %s, only for %s"
                % (self.chcfg["type"], phrst_gens)
            )
        ...
        if self.prog.ABSOLUTE_FREQS and self.chcfg["has_mixer"]:
            ...
            f_dds = par["freq"]
            f_offset = 0
        w["freqreg"] = self.prog.freq2reg(
            gen_ch=self.ch, f=f_dds, ro_ch=par.get("ro_ch")
        )
        ...
        if par["style"] == "flat_top":
            ...
            w["gainreg"] = to_int(
                ...
                self.chcfg["maxv"],
                parname="gain",
                quantize=flat_scale.denominator,
                trunc=True,
            )
            ...
            w["gainreg"] = to_int(
                par["gain"],
                self.chcfg["maxv"] * self.chcfg["maxv_scale"],
                parname="gain",
                trunc=True,
            )
        else:
            w["gainreg"] = to_int(
                par["gain"], self.chcfg["maxv"], parname="gain", trunc=True
            )
        ...
        if par["style"] == "const":
            ...
            if env_length % 2 != 0:
                logger.warning(
                    "Envelope length %d is an odd number of fabric cycles.\n"
    ...
    def params2wave(self, maskreg, lenreg):
        ...
        if isinstance(lenreg, QickRawParam):
            if lenreg.maxval() >= 2**32 or lenreg.minval() < 3:
                raise RuntimeError(
                    "Pulse length of %d cycles is out of range (exceeds 32 bits, or less than 3) - use multiple pulses, or zero-pad the envelope"
                    % (lenreg)
                )
        else:
    ...
    def params2pulse(self, par):
        ...
        for maskch in par["mask"]:
            if maskch not in range(len(tones)):
                raise RuntimeError(
                    "mask includes tone %d, but only %d tones are declared"
                    % (maskch, len(tones))
                )
            maskreg |= 1 << maskch
...
class ReadoutManager(AbsRegisterManager):
    ...
    def params2pulse(self, par):
        ...
        if self.prog.ABSOLUTE_FREQS:
            ...
            f_dds = par["freq"]
        freqreg = self.prog.freq2reg_adc(
            ro_ch=self.ch, f=f_dds, gen_ch=par.get("gen_ch")
        )
...
class QickProgramV2(AsmV2, AbsQickProgram):
    ...
    def __init__(self, soccfg):
        ...
        if self.tproccfg["type"] != "qick_processor":
            raise RuntimeError(
                "tProc v2 programs can only be run on a tProc v2 firmware"
            )
        if self.tproccfg["revision"] != ASM_REVISION:
            raise RuntimeError(
                "this version of the QICK library only supports tProc v2 revision %d, you have %d"
                % (ASM_REVISION, self.tproccfg["revision"])
            )
    ...
    def _init_declarations(self):
        ...
        self._gen_mgrs = [
            self.gentypes[ch["type"]](self, iCh)
            for iCh, ch in enumerate(self.soccfg["gens"])
        ]
        self._ro_mgrs = [
            ReadoutManager(self, iCh) if "tproc_ctrl" in ch else None
            for iCh, ch in enumerate(self.soccfg["readouts"])
        ]
    ...
    def _make_asm(self):
        ...
        for macro in self.macro_list:
            ...
            if (
                isinstance(macro, TimedMacro)
                and hasattr(macro, "tag")
                and macro.tag is not None
            ):
                if macro.tag in self.time_dict:
                    raise RuntimeError(
                        "two instructions have the same tag %s" % (macro.tag)
                    )
    ...
    def get_pulse_param(self, pulsename, parname, as_array=False):
        ...
        if self.binprog is None:
            raise RuntimeError(
                "get_pulse_param() can only be called on a program after it's been compiled"
            )

        pulse = self.pulses[pulsename]
        if parname not in pulse.numeric_params:
            raise RuntimeError(
                "invalid parameter name; use list_pulse_params() to get the list of valid names for this pulse"
            )
        ...
        if as_array:
            return param.to_array(self.loop_dict)
        elif param.is_sweep():
            return param
        else:
            return float(param)
    ...
    def add_reg(
        ...
        if allow_reuse:
            if init is not None or addr is not None:
                raise ValueError(
                    "for allow_reuse=True, init and addr parameters must be left as None"
                )
            if name in self.reg_dict:
                if self.reg_dict[name].init is not None:
                    raise RuntimeError(
                        "for allow_reuse=True, previously allocated register must not have an init value"
                    )
                return name
        ...
        if addr is None:
            ...
            if addr < 0 or addr >= self.soccfg["tprocs"][0]["dreg_qty"]:
                raise ValueError(
                    f"register address must be smaller than {self.soccfg['tprocs'][0]['dreg_qty']}"
                )
        ...
        if name in self.REG_ALIASES:
            raise NameError(
                f"register name '{name}' is reserved for use as a register alias"
            )
            ...
            raise NameError(
                f"requested name {name} is reserved for use as a register address"
            )
...
class AveragerProgramV2(AcquireProgramV2):
    ...
    def compile(self):
        ...
        self.setup_acquire(
            ...
            loop_dims=[x[1] for x in self.loops],
            avg_level=0,
        )
